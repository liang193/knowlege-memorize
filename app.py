        ...
        if not kb_names:
            ...
            if st.button("创建", use_container_width=True):
                ...
                if ok:
                    ...
                    st.error(msg)
            return
        selected_kb = st.selectbox("选择知识库", kb_names, index=kb_names.index(st.session_state.current_kb) if st.session_state.current_kb in kb_names else 0, key="kb_selector")
                    ...
                    if full_text:
                        ...
                        for p in file_data.get("paragraphs", []):
                            ...
                            if text and len(text) > 10:
                                card = sm2.init_card(text[:60], text, source_text=text)
